...
impl Cell {
  pub fn get_face(&self, side: Border) -> Face {
    match self {
      Cell::CornerCell(option_prop, borders) => Cell::get_face_corner(side, option_prop, borders),
      Cell::BorderCell(option_prop, border) => Cell::get_face_border(side, option_prop, border),
      Cell::FullCell(
        Some(Props {
